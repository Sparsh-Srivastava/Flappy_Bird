...
App.Main.prototype = {
  preload: function () {
    ...
    this.load.bitmapFont(
      "fnt_chars_black",
      "assets/fnt_chars_black.png",
      "assets/fnt_chars_black.fnt"
    );
    this.load.bitmapFont(
      "fnt_digits_blue",
      "assets/fnt_digits_blue.png",
      "assets/fnt_digits_blue.fnt"
    );
    this.load.bitmapFont(
      "fnt_digits_green",
      "assets/fnt_digits_green.png",
      "assets/fnt_digits_green.fnt"
    );
    this.load.bitmapFont(
      "fnt_digits_red",
      "assets/fnt_digits_red.png",
      "assets/fnt_digits_red.fnt"
    );
  },
  ...
  create: function () {
    ...
    this.txtPopulationPrev = new Text(
      this.game,
      1190,
      10,
      "",
      "right",
      "fnt_chars_black"
    ); // No. of the previous population
    this.txtPopulationCurr = new Text(
      this.game,
      1270,
      10,
      "",
    ...
    for (var i = 0; i < this.GA.max_units; i++) {
      ...
      this.txtStatusPrevGreen.push(
        new Text(this.game, 1190, y, "", "right", "fnt_digits_green")
      );
      this.txtStatusPrevRed.push(
        new Text(this.game, 1190, y, "", "right", "fnt_digits_red")
      );
      this.txtStatusCurr.push(
        new Text(this.game, 1270, y, "", "right", "fnt_digits_blue")
      );
    }

    // create a text object displayed in the HUD footer to show info of the best unit ever born
    this.txtBestUnit = new Text(
      this.game,
      1095,
      580,
      "",
    ...
    switch (this.state) {
      ...
      case this.STATE_START: // start/restart the game
        ...
        this.txtBestUnit.text =
          "The best unit was born in generation " +
          this.GA.best_population +
          ":" +
          "\nFitness = " +
          this.GA.best_fitness.toFixed(2) +
          " / Score = " +
          this.GA.best_score;
        ...
        this.lastBarrier = this.BarrierGroup.getAt(
          this.BarrierGroup.length - 1
        );
      ...
      case this.STATE_PLAY: // play Flappy Bird game by using genetic algorithm AI
        ...
        this.BirdGroup.forEachAlive(function (bird) {
          ...
          bird.fitness_curr =
            this.distance -
            this.game.physics.arcade.distanceBetween(bird, this.TargetPoint);
        ...
        if (this.firstBarrier.getWorldX() < -this.firstBarrier.width) {
          this.firstBarrier.restart(
            this.lastBarrier.getWorldX() + this.BARRIER_DISTANCE
          );
    ...
    this.BirdGroup.forEach(function (bird) {
      ...
      if (bird.alive) {
        ...
        this.bmdStatus.rect(
          ...
          y,
          9,
          -(50 - brain.neurons[0].activation / scale),
          "#000088"
        ); // input 1
        this.bmdStatus.rect(
          90,
          y,
          9,
          brain.neurons[1].activation / scale,
          "#000088"
        ); // input 2

        if (brain.neurons[brain.neurons.length - 1].activation < 0.5)
          this.bmdStatus.rect(118, y, 9, -20, "#880000"); // output: flap = no
        else this.bmdStatus.rect(118, y, 9, -40, "#008800"); // output: flap = yes
      }

      // draw bird's fitness and score
      this.txtStatusCurr[bird.index].setText(
        bird.fitness_curr.toFixed(2) + "\n" + bird.score_curr
      );
    }, this);
  },
